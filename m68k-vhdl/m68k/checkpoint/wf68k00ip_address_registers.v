...
`include "wf68k00ip_parms.h"
   ...
   assign usp_out = usp;
   assign ssp_out = ssp;
   assign pc_out = pc + pc_tmp; // Plus offset.
   ...
   always @ (posedge clk or negedge resetn )
	...
	if (~resetn)
          begin
	     /* variable_assignment  extend_index__index_tmp  has moved. */
	  end 
	else 
   ...
   always @ (clk or resetn or ar_nr_i or i_d_a or i_w_l or
...
//
//                   index_ext[15:0] = ar[ar_nr_i][15:0];
     ...
     begin 
        if (~resetn)
          ...
          if (pc_inc | pc_tmp_clr)
            ...
            pc_tmpvar = pc_tmpvar + 1'b1;
        
        pc_tmp <= {pc_tmpvar,1'b0}; // Increment always by two.
     end 
   ...
   always @ (negedge resetn or posedge clk )
	...
	if (~resetn)
	     ...
	     // Increment / decrement and initialise the stack:
             if (usp_inc) 
               usp <= usp + 2'b10; // Increment 2 bytes.
             
             else if (usp_dec) 
               usp <= usp - 2'b10; // Decrement 2 bytes.
   ...
   always @ (resetn or clk or op or ar_nr_a or ar_nr_b)
     if (~resetn)
       begin 
...
`ifdef debug
   always @ (posedge clk)
     if (resetn)
       begin
	  if (pc_init)
	    $display("PC <- %x", adata_in);
	  if (ssp_init)
	    $display("SSP <- %x", adata_in);
       end
`endif
endmodule
