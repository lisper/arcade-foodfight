...
//brad
// exg, lea, move_usp, movec, moveq
//		      (op_i == MOVEQ | op_i == LEA | op_i == EXG) ? LONG : // unsized
//brad
...
`ifdef debug
   ...
   always @ (posedge clk)
	 ...
	 begin
	    if (src_destn)
	      $display("ew_wr: exword_regx ew_adr %x data_in %x", ew_adr, data_in);
	    else
	      $display("ew_wr: dest_exword_regx ew_adr %x data_in %x", ew_adr, data_in);
	 end
     end
